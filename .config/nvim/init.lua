...
vim.keymap.set('n', '<leader>q', vim.diagnostic.setloclist, { desc = 'Open diagnostic [Q]uickfix list' })
--vim.keymap.set('t', '<Esc><Esc>', '<C-\\><C-n>', { desc = 'Exit terminal mode' })
-- See if this is really so intrusive
...
local function p4_diff_prev()
  ...
  local result = vim.fn.system('p4 diff ' .. current_file)
  if vim.v.shell_error ~= 0 then
    print(result)
    return
  end
  ...
  local temp_file = vim.fn.tempname() .. '_' .. vim.fn.expand '%:t'
  local p4cmd = string.format('p4 print -q -o %s %s#have', temp_file, current_file)
  result = vim.fn.system(p4cmd)
  if vim.v.shell_error ~= 0 then
    print('Error getting previous revision: ' .. result)
    return
  end
